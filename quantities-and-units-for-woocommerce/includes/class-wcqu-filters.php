...
class WC_Quantities_and_Units_Filters {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_rest_prepare_product',array($this,'add_rules_to_product_api'), 90, 2 );
		add_filter( 'woocommerce_rest_prepare_product',array($this,'add_units_to_product_api'), 90, 2 );

	}

	public function add_rules_to_product_api( $response, $post ) {
		// retrieve a custom field and add it to API response
		$response->data['rules'] = wcqu_get_value_from_rule( 'all', wc_get_product($post->ID), wcqu_get_applied_rule(wc_get_product($post->ID)) );
		return $response;
	}

	public function add_units_to_product_api( $response, $post ) {

		// retrieve a custom field and add it to API response
		$product_unit = wp_get_post_terms( $post->ID,'product_unit' )[0];
		$response->data['unit']->id =  $product_unit->term_id;
		$response->data['unit']->name =  $product_unit->name;
		$response->data['unit']->slug = $product_unit->slug;
