...
function wcqu_get_applied_rule_obj( $product, $role = null ) {
	...
	$product_units = wp_get_post_terms( $product->id, 'product_unit' );
	...
	$product_terms = array_merge( $product_cats, $product_tags,$product_units );
	...
	foreach ( $rules as $rule ) {
		...
		$units = get_post_meta( $rule->ID, '_units' );
		...
		if ( $units != false )
	 	...
	 	$rule_taxes = array_merge( $tags, $cats,$units );
