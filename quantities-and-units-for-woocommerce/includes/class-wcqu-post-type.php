...
//		add_action( 'init', array( $this, 'units_init' ) );

			// Adjust post type columns on list view
			add_action( 'manage_edit-quantity-rule_columns', array( $this, 'quantity_rule_columns' ), 10, 2 );
			add_action( 'manage_quantity-rule_posts_custom_column', array(
				$this,
				'manage_quantity_rule_columns'
			), 10, 2 );
			add_filter( 'manage_edit-quantity-rule_sortable_columns', array(
				$this,
				'sortable_quantity_rule_columns'
			) );
			...
			add_action( 'add_meta_boxes', array( $this, 'units_init' ) );
...
//		add_action( 'add_meta_boxes', array( $this, 'input_thumbnail_notice' 	) );
			...
			add_action( 'save_post', array( $this, 'save_quantity_rule_taxes_units' ) );
			add_action( 'init', array( $this, 'create_units_taxonomy' ), 0 );
		}

		public function create_units_taxonomy() {
			register_taxonomy( 'product_unit',
				apply_filters( 'woocommerce_taxonomy_objects_product_unit', array( 'product' ) ),
				apply_filters( 'woocommerce_taxonomy_args_product_unit', array(
					'hierarchical'          => false,
					'update_count_callback' => '_wc_term_recount',
					'label'                 => __( 'الوحدات', 'woocommerce' ),
					'labels'                => array(
						'name'          => __( 'وحدات المنتج', 'woocommerce' ),
						'singular_name' => __( 'الوحدة', 'woocommerce' ),
						'menu_name'     => _x( 'الوحدات', 'Admin menu name', 'woocommerce' ),
						'search_items'  => __( 'بحث في الوحدات', 'woocommerce' ),
						'all_items'     => __( 'جميع الوحدات', 'woocommerce' ),
						'edit_item'     => __( 'تعديل الوحدة', 'woocommerce' ),
						'update_item'   => __( 'تحديث الوحدة', 'woocommerce' ),
						'add_new_item'  => __( 'اضافة وحدة جديدة', 'woocommerce' ),
						'new_item_name' => __( 'اسم الوحدة الجديدة', 'woocommerce' ),
						'not_found'     => __( 'لم يتم العثور على وحدة', 'woocommerce' ),
					),
					'show_ui'               => true,
					'query_var'             => true,
					'capabilities'          => array(
		...
		*/
		public function quantity_rule_init() {
			...
			$labels = array(
				'name'               => 'قوانين الكمية',
				'singular_name'      => 'قانون الكمية',
				'add_new'            => 'اضافة جديد',
				'add_new_item'       => 'اضافة قانون جديد',
				'edit_item'          => 'تعديل قانون',
				'new_item'           => 'قانون جديد',
				'all_items'          => 'جميع القوانين',
				'view_item'          => 'عرض القانون',
				'search_items'       => 'بحث في القوانين',
				'not_found'          => 'لا توجد قوانين',
				'not_found_in_trash' => 'لا توجد قوانين في السلة',
				'parent_item_colon'  => '',
				'menu_name'          => 'قوانين الكميات'
			);
...
//    }
		...
		*/
		public function quantity_rule_columns( $column ) {
			...
			$new_columns['priority'] = __( 'الاولوية' );
			$new_columns['min']      = __( 'الادنى' );
			$new_columns['max']      = __( 'الاقصى' );
			$new_columns['step']     = __( 'كمية الزيادة' );
			$new_columns['cats']     = __( 'التصنيفات' );
			$new_columns['units']    = __( 'الوحدات' );

			$new_columns['product_tags'] = __( 'الملحقات' );
			$new_columns['roles']        = __( 'الصلاحيات' );
			$new_columns['date']         = __( 'التاريخ' );
		...
		public function manage_quantity_rule_columns( $column_name, $id ) {
			...
			switch ( $column_name ) {
				...
				case 'cats':
					...
					break;
				case 'units':
					$units = get_post_meta( $id, '_units', false );
					if ( $units != false and count( $units[0] ) > 0 ) {
						foreach ( $units[0] as $unit ) {

							$taxonomy = 'product_unit';
							$term     = get_term_by( 'id', $unit, $taxonomy );
							$link     = get_term_link( $term );
		...
		*/
		...
		public function quantity_rule_meta( $post ) {
			...
			?>
            <div class="wpbo-meta">
                <label for="min">حد ادنى</label>
                <input type="number" name="min" id="min" value="<?php echo $min ?>" step="any"/>

                <label for="max">حد اقصى</label>
                <input type="number" name="max" id="max" value="<?php echo $max ?>" step="any"/>

                <label for="_wpbo_minimum_oos">الحد الادنى للمخزون</label>
                <input type="number" name="min_oos" value="<?php echo $min_oos ?>" step="any"/>

                <label for="_wpbo_maximum_oos">الحد الاقصى للمخزون</label>
                <input type="number" name="max_oos" value="<?php echo $max_oos ?>" step="any"/>

                <label for="step">كمية الزيادة (مع كل ضغطة)</label>
                <input type="number" step="any" name="step" id="step" value="<?php echo $step ?>" step="any"/>

                <label for="step">الاولوية</label>
                <input type="number" name="priority" id="priority" value="<?php echo $priority ?>"/>
            </div>
            
            <p><em>ملاحظة : الحد الادنى لا بد ان يكون اكبر من او مساوي لكمية الزيادة*</em><br/>
                <em> ملاحظة القانون صاحب الاولوية الاقل هو الذي سيتم استخدامه في حالة تواجد اكثر من قانون لنفس المنتج</em></p>
			<?php
		}
		...
		*/
		...
		public function units_init() {
			add_meta_box(
				'wpbo-quantity-rule-tax-meta_unit',
				'الوحدات',
				array( $this, 'quantity_rule_tax_meta_unit' ),
				'quantity-rule',
				'normal',
				'high'
			);
		...
		function quantity_rule_tax_meta_unit( $post ) {

			// Get selected categories
			$units = get_post_meta( $post->ID, '_units', false );
			...
			if ( $terms ) {
				...
				foreach ( $terms as $term ) {
					$this->print_tax_inputs_units( $term, $tax_name, $units, 2 );
				}
				echo '</ul>';
			}
		}
		...
		*/
		public function print_tax_inputs( $term, $taxonomy_name, $cats, $level ) {
			...
			?>
            <li>
                <input type="checkbox" id="_wpbo_cat_<?php echo $term->term_id ?>"
                       name="_wpbo_cat_<?php echo $term->term_id ?>" <?php if ( is_array( $cats ) and in_array( $term->term_id, $cats ) )
					echo 'checked="checked"' ?> /><?php echo $term->name; ?>
            </li>
			<?php
		...
		public function print_tax_inputs_units( $term, $taxonomy_name, $units, $level ) {

			// Echo Single Item
			?>
            <li>
                <input type="checkbox" id="_wpbo_unit_<?php echo $term->term_id ?>"
                       name="_wpbo_unit_<?php echo $term->term_id ?>" <?php if ( is_array( $units ) and in_array( $term->term_id, $units ) )
					echo 'checked="checked"' ?> /><?php echo $term->name; ?>
            </li>
			<?php
			...
			if ( $children ) {
				...
				foreach ( $children as $child_id ) {
					...
					// If the child is at the second level relative to the last printed element, exclude it
					if ( is_object( $child ) and $child->parent == $term->term_id ) {
						$this->print_tax_inputs( $child, $taxonomy_name, $units, $level );
					}
				}
				echo '</ul>';
			}
			...
			if ( $tags ) {
                ...
                <ul id='product-tags' class='rule-product-cats'>
					<?php foreach ( $tags as $tag ): ?>
                        <li>
                            <input type='checkbox' name='_wpbo_tag_<?php echo $tag->term_id ?>'
                                   id='wpbo_tag_<?php echo $tag->term_id ?>' <?php if ( in_array( $tag->term_id, $included_tags ) )
								echo 'checked' ?> />
							<?php echo $tag->name ?>
                        </li>
					<?php endforeach; ?>
			...
			if ( $roles ): ?>
                <ul>
					<?php foreach ( $roles as $slug => $name ): ?>
                        <li>
                            <input type='checkbox' name='_wpbo_role_<?php echo $slug ?>'
                                   id='wpbo_role_<?php echo $slug ?>' <?php if ( in_array( $slug, $applied_roles ) || empty( $applied_roles ) )
								echo 'checked' ?> />
							<?php echo $name; ?>
                        </li>
					<?php endforeach; ?>
                    <li>
                        <input type='checkbox' name='_wpbo_role_guest'
                               id='wpbo_role_guest' <?php if ( in_array( 'guest', $applied_roles ) || empty( $applied_roles ) )
							echo 'checked' ?> />
                        Guest
                    </li>
                </ul>
		...
		*/
		...
		public function company_notice_meta( $post ) {
			...
			?>
            <a href="http://rapidorderplugin.com/?utm_source=QU%20Plugin%20Admin&utm_medium=sidebar&utm_campaign=Fast%20order%20form%20for%20WooCommerce"
               target="_blank"><img align="center"
                                    src="<?php echo plugins_url() ?>/quantities-and-units-for-woocommerce/assets/img/rapid-order-logo.png"/></a>
            <p>
                <a href="http://rapidorderplugin.com/?utm_source=QU%20Plugin%20Admin&utm_medium=sidebar&utm_campaign=Fast%20order%20form%20for%20WooCommerce"
                   target="_blank">Fast order form for WooCommerce</a>
            </p>
			<?php
		}
	...
	*/
		...
		public function additional_info_notice_meta( $post ) {
            ...
            <div style="text-align: center">
                ...
                <a href="http://wordpress.org/support/view/plugin-reviews/quantities-and-units-for-woocommerce"
                   target="_blank">Rate us on Wordpress.org!</a>

                <h3>Need Support?</h3>
                <a href="http://wordpress.org/support/plugin/quantities-and-units-for-woocommerce" target="_blank">Visit
                    our Support Forum</a>
            </div>
			<?php
		}
		...
		*/
		public function save_quantity_rule_taxes_units( $post_id ) {
			...
			foreach ( $terms as $term ) {
				$term_name = '_wpbo_unit_' . $term->term_id;
				if ( isset( $_POST[ $term_name ] ) and $_POST[ $term_name ] == 'on' ) {
					array_push( $units, $term->term_id );
				}
			}

			// Add them to the post meta
			delete_post_meta( $post_id, '_units' );
			update_post_meta( $post_id, '_units', $units, false );
		}
