...
function add_scripts(){
     wp_register_script( 'checkout', plugins_url( '/assets/js/checkout.min.js', dirname(__FILE__) ) );
	...
	wp_localize_script( 'checkout', 'wc_checkout_params', $data );
	
}

$data = preg_replace('/[\/\\\]/', '',urldecode($_GET['order']));
$data = json_decode($data, true);
  ...
  list($billing['city'],$billing['state']) = array($billing['state'],$billing['city']);
     $shipping = $data['shipping'];
      list($shipping['city'],$shipping['state']) = array($shipping['state'],$shipping['city']);
    $meta_data = $data['meta_data']; 
    $delivery_date = '';
    $delivery_time = '';
    foreach($meta_data as $item){
        if ($item['key'] == '_delivery_date'){
            $delivery_date = date('d-m-Y',$item['value']);
        }
        if ($item['key'] == '_delivery_time'){
    ...
    <script type="text/javascript">
        jQuery(document).ready(function($){
            ...
            var delivery_time= '<?php echo $delivery_time ?>';
            var st = delivery_date.replace(/-/g,'.');
            var pattern = /(\d{2})\.(\d{2})\.(\d{4})/;
            var dt = new Date(st.replace(pattern,'$3-$2-$1'));
            $('#delivery_date').datepicker("setDate", dt );
            var myOpts = document.getElementById('delivery_time').options;
            for (var i = 0 ; i < myOpts.length ; i++){
               if (myOpts[i].value == delivery_time ){
        ...
        jQuery(window).bind("load", function() {
           var phone = '<?php echo $billing['phone'] ?>';
